...
class ValidateUserLogin(object):
    ...
    def __init__(self, login_data):
        self.esummit_id = login_data.get("esummit_id", "")
    ...
    def _get_user(self):
        ...
        return User.objects.filter(esummit_id=self.esummit_id, email=self.email).first()
...
class UserLogin(object):
    ...
    def _create_login(self):
        # user = self._get_user()
        # check this part. when deletes the data, just session token is cleared, entry is still there.
        # if attempt to make a new one, will it replace the earlier one with same user id, or create another one. 
        login_data = {
